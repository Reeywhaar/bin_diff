...
fn combine_diffs_to_blocks<'a, T: 'a + Read + Seek, U: 'a + Read + Seek>(
	diff_a: T,
	diff_b: U,
...
pub fn combine_diffs<'a, T: 'a + Read + Seek, U: 'a + Read + Seek, W: Write>(
	...
	let out = combine_diffs_to_blocks(&mut diff_a, &mut diff_b)?;
...
// pub fn combine_diffs_vec<'a, T: 'a + Read + Seek>(diffs: &mut [T]) -> IOResult<()> {
// 	let diff = &mut diffs[0];
// 	for
// 	Ok(())
// }
